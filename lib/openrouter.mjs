...
export function buildMessages({ paper, question, history, chunks }) {
  ...
  const system = [
    ...
    "Fasse dich auf höchstens 350 Wörter und schließe jeden Satz vollständig ab.",
...
export function extractAnswer(payload) {
  ...
  if (choice?.finish_reason === "length") {
    throw Object.assign(new Error("OpenRouter truncated the answer"), { status: 502 });
  }
...
export async function answerWithModel({ env, paper, question, history, chunks }) {
  ...
  const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.1,
      max_tokens: Number(env.MODEL_MAX_TOKENS ?? 3000),
      stream: false,
    }),
  ...
  const trimmed = extractAnswer(payload);
